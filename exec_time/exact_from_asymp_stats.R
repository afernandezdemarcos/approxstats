exact_quantile_from_asymp <- function(T_inf, statistic, n, alpha, Stephens = FALSE){
  if(statistic=="KS"){exact.stat <- T_inf / Kuiper_mod(T_inf, n, alpha, 
                                                                    KS = TRUE, Stephens = Stephens)}
  if(statistic=="Kuiper"){exact.stat <- T_inf / Kuiper_mod(T_inf, n, alpha, 
                                                                        KS = FALSE, Stephens = Stephens)}
  if(statistic=="CvM"){exact.stat <- T_inf / Watson_mod(T_inf, n, alpha, 
                                                                     CvM = TRUE, Stephens = Stephens)}
  if(statistic=="Watson"){exact.stat <- T_inf / Watson_mod(T_inf, n, alpha, 
                                                                        CvM = FALSE, Stephens = Stephens)}
  if(statistic=="AD"){exact.stat <- T_inf / AndersonDarling_mod(T_inf, n, alpha, 
                                                                             Stephens = Stephens)}
  return(exact.stat)
}

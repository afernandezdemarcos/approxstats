...
D.exec_time <- array(
  dim = c(length(n.list), length(alpha.list), 2),
  dimnames = list(n.list, alpha.list, NULL)
)
W2.exec_time <- array(
  dim = c(length(n.list), length(alpha.list), 3),
  dimnames = list(n.list, alpha.list, NULL)
)
A2.exec_time <- array(
  dim = c(length(n.list), length(alpha.list), 2),
...
p_cir_stat_CvM <- function(T_n, n) {
  ...
  m <- length(alpha_grid)
  T_inf <- c(
    1.167869, 1.03846, 0.9633351, 0.9103154, 0.8693864, 0.8360821,
  ...
  for (i in 1:m) {
    ...
    if (T_mod > T_inf[i]) {
      if (i > 1) {
        alpha <- newton_raphson_quadratic_R(
          T_n, n, -0.1651, 0.0749, -0.0014,
...
for (n in n.list) {
  ...
  for (alpha in alpha.list) {
    D_n <- exact_quantile_from_asymp(
      T_inf = D_inf[[as.character(alpha)]],
      statistic = "KS", n = n, alpha = alpha
    )
    W2_n <- exact_quantile_from_asymp(
      T_inf = W2_inf[[as.character(alpha)]],
      statistic = "CvM", n = n, alpha = alpha
    )
    A2_n <- exact_quantile_from_asymp(
      T_inf = A2_inf[[as.character(alpha)]],
...
for (n in n.list) {
  ...
  KS_MC <- summary(
    microbenchmark(
      "MC" = {
        MC_Tn <- unif_stat_MC(
          n = n, statistic = "KS", p = 2, M = 1e4,
          cores = 1, return_stats = TRUE
        )$stats_MC
        mean(D_n > MC_Tn)
      },
      unit = "us",

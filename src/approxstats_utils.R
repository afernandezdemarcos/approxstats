...
compute_pdf <- function(T_n) {
  ...
  pdf_Tn <- matrix(
    nrow = length(n_vector), ncol = length(alpha_vector),
...
get_formula_lm <- function(model, intercept = TRUE, precision = 4) {
  if (intercept) {
    ...
    form <- paste0(
      round(coefficients(model)[1], precision), " + ",
      paste(sprintf(
        paste0("%.", precision, "f * %s"),
        coefficients(model)[-1],
        names(coefficients(model)[-1])
...
process_MC_results <- function(results, B = 100, ci.limits = c(0.025, 0.975)) {
  ...
  rejection.results <- array(
    dim = results.dim[2:length(results.dim)],
    dimnames = subsetList(
      results.dimnames,
  ...
  return(list(
    rejection.results = rejection.results,

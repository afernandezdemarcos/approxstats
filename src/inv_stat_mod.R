...
inv_cir_stat_mod <- function(type, asymp_statistic, n, alpha, Stephens = FALSE) {
  if (type == "KS") {
    mod.stat <- inv_cir_stat_Kuiper_mod(asymp_statistic, n, alpha,
  ...
  if (type == "CvM") {
    mod.stat <- inv_cir_stat_Watson_mod(asymp_statistic, n, alpha,
  ...
  if (type == "AD") {
    mod.stat <- inv_cir_stat_AD_mod(asymp_statistic, n, alpha,
      Stephens = Stephens
    )
  }

  if (type == "PAD") {
    mod.stat <- inv_cir_stat_PAD_2_mod(asymp_statistic, n, alpha)
  }
...
inv_sph_stat_mod <- function(type, asymp_statistic, n, alpha, p) {
  if (type == "PCvM") {
    mod.stat <- inv_sph_stat_PCvM_mod(asymp_statistic, n, alpha, p)
  }

  if (type == "PAD") {
    mod.stat <- inv_sph_stat_PAD_mod(asymp_statistic, n, alpha, p)
  }

  if (type == "Bakshaev") {
    mod.stat <- inv_sph_stat_Bakshaev_mod(asymp_statistic, n, alpha, p)
  }

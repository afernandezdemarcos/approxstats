...
fit_T_Tn <- function(T_Tn, T_n, T_inf, f_T_n,
  ...
  for (i in 1:length(vars)){
    ...
    if("n" %in% var){n.var.set <- c(n.var.set, var.name)}
    if("alpha" %in% var){alpha.var.set <- c(alpha.var.set, var.name)}
  ...
  if(!fit.alpha){
    ...
    # (n, alpha)-model
    saturated.form <- formula(paste0("T_Tn ~ ", 
                                     "0 + ", form.n, 
                                     " + (", form.n, "):(", form.alpha, ")"))
  ...
  if(model.reference == "Stephens"){
    ...
    T_Tn_initial <- lm(formula = formula("T_Tn ~ 0 + inv_n_2_2 + inv_n_1_2"), 
                       data, 
                       weights = weights.vec) 
    ...
    T_Tn_initial.form <- formula(paste("T_Tn ~ 0 + ", 
                                       paste(lasso.preds, 
                                             collapse = " + ")
                                       )
                                 )
    ...
    results[["Lasso"]] <- list(kcv = lasso.fit$fit, 
...
get_lasso_predictors <- function(lasso.fit, opt.lambda="1se"){
  ...
  sel.preds <- predict(lasso.fit$fit, type = "coefficients", 

...
for (p in p.list) {
  ...
  asymp.quantiles <- quantiles[
    (as.numeric(rownames(quantiles) >= q.start)) &
...
T_Tn_manual <- fit_T_Tn(
  T_Tn = NULL, T_inf = NULL, T_n = NULL, f_T_n = NULL,
  fit.direction = "both",
  N.start = 5,
  fit.p = fit.p,
  data = data,
...
new.formula <- drop_term_r_squared(model,
  delta.max = 1e-3,
  R2_0 = R2_original, scope = NULL
)
i <- 0
while (!is.null(new.formula)) {
  T_Tn_particular <- fit_T_Tn(
    T_Tn = T_Tn, T_n = T_n,

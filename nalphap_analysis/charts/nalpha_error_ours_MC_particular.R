...
method_filename.list <- list(
  "CvM" = "Csorgo_and_Faraway_1996",
  "KS" = "Marsaglia_et_al_2003",
  "AD" = "Marsaglia_and_Marsaglia_2004"
)
method_name.list <- list(
  "CvM" = "Csörgo & Faraway (1996)",
...
for (statistic in statistic.list) {
  ...
  rel.error.steph.summ <- matrix(
    nrow = length(bins), ncol = length(alpha),
    dimnames = list(bins, alpha)
  )
  rel.error.part.summ <- matrix(
    nrow = length(bins), ncol = length(alpha),
    dimnames = list(bins, alpha)
  )
  rel.error.ours.summ <- matrix(
    nrow = length(bins), ncol = length(alpha),
    dimnames = list(bins, alpha)
  )
  rel.error.MC.summ <- matrix(
    nrow = length(bins), ncol = length(alpha),
    dimnames = list(bins, alpha)
  )
  ...
  # Plot setup
  pdf(
    file = paste0("results/rel_error_alpha_", M.name, "_", statistic, ".pdf"),
    width = 6, # 7
    height = 6
  ) # 5

...
colors <- c("#E74C3C", "#9B59B6", "#3498DB", "#27AE60", 
            "#F1C40F", "#E67E22", "#5D6D7E")

for (statistic in c("PCvM", "Bakshaev", "PAD")){
  
  print(paste0("<======== ", statistic, " ========>"))
  errors.summary <- matrix(nrow = length(p.list), ncol = length(alpha.list), 
                           dimnames = list(p.list, alpha.list))
  errors.summary.MC <- matrix(nrow = length(p.list), ncol = length(alpha.list), 
                              dimnames = list(p.list, alpha.list))
  ...
  for (p in p.list){
    ...
    error.MC.m <- array(0, dim = dim(mod.results.MC), 
    ...
    errors.summary[as.character(p), ] <- apply(rel.error, 2, 
                                               mean)[as.character(alpha.list)]
    errors.summary.MC[as.character(p), ] <- apply(rel.error.MC, 2, 
                                                  function(x) mean(x, 
                                                                   na.rm = TRUE)
  ...
  if (plot.errors){
    ...
    # Plot setup
    pdf(file = paste0("results/rel_error_p_", M.name, "_", statistic, ".pdf"),
        width = 6,#8
        height = 6)#6.7
    ...
    if (statistic == "PCvM"){
      leg.1 <- paste("expression(paste(alpha, \": \", ", 

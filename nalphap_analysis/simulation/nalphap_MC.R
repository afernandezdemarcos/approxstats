...
crit_val_MC <- array(
  dim = c(length(n_list), length(alpha_list), length(p_list), replicates),
  dimnames = list(n_list, alpha_list, p_list, NULL)
)

mod.results <- array(
  dim = c(length(n_list), length(alpha_list), length(p_list), replicates, length(statistic_list)),
...
for (p in p_list) {
  ...
  for (n in n_list) {
    ...
    unif_stat_MC_n <- unif_stat_MC(n,
      type = statistic_list, p = p, return_stats = TRUE,
    ...
    for (statistic in statistic_list) {
      ...
      crit_val <- rbind(apply(stat_MC_n, 2, quantile,
        probs = 1 - alpha_list,
        na.rm = TRUE
      ))
...
for (statistic in statistic_list) {
  ...
  save(save.res, file = paste0(
    "results/crit_val_MC_", M, "_",
    statistic, "_MC.RData"
  ))
}

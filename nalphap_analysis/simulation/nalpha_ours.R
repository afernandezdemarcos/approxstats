...
samples <- array(
  dim = c(M, length(statistic.list), length(n.list)),
...
for (n in n.list) {
  ...
  cir <- unif_stat_MC(
    n = n, M = M, type = statistic.list, p = 2,
    chunks = 100, seeds = 1:100, cores = 3
  )$stats_MC
...
for (statistic in statistic.list) {
  ...
  for (Stephens in Stephens.list) {
    ...
    mod.results <- array(
      dim = c(M, length(n.list), length(alpha.list)),
    ...
    for (alpha in alpha.list) {
      ...
      for (n in n.list) {
        ...
        if (Stephens == "particular_method") {
          ...
          cir.mod <- unlist(lapply(
            cir,
            function(x) {
              return(cir_stat_mod(type = statistic, x, n, alpha, Stephens = as.logical(Stephens)))
            }
          ))

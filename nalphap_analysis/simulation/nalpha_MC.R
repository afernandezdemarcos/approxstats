...
crit_val_MC <- array(
  dim = c(length(n_list), length(alpha_list), replicates),
  dimnames = list(n_list, alpha_list, NULL)
)

mod.results <- array(
  dim = c(length(n_list), length(alpha_list), replicates, length(statistic_list)),
...
for (n in n_list) {
  ...
  unif_stat_MC_n <- unif_stat_MC(n,
    type = statistic_list, p = 2, return_stats = TRUE,
    cores = 3, M = M.mc
  )

...
bw0 <- rotation.k
bw0.list <- list("21" = list("N" = 10.26768, "S" = 10.82924, "all" = 5.483238),
...
colors <- list("N" = rgb(red = 18, green = 93, blue = 152, 
                         alpha = 80, maxColorValue = 255), 
               "S" = rgb(red = 177, green = 99, blue = 110, 
                         alpha = 80, maxColorValue = 255),
               "all" = rgb(red = 0, green = 0, blue = 0, 
                           alpha = 80, maxColorValue = 255))

line.colors <- list("N" = rgb(red = 18, green = 93, blue = 152, 
...
for (obs.window in obs.window.list){
  ...
  print(paste0("<======== Obs. window: ", 
               obs.window/carrington.rotation, " ========>"))
  
  i <- i + 1
  if (save.plots) pdf(paste0("sunspots/results/sunspots_cir_", statistic,
  ...
  for (cycle in cycles){
    ...
    par(mar = c(-0.1, if(cycle == 21) 4 else -0.1, -0.1,
                if(cycle == 23) 2 else -0.1) + 0.1)
    
    plot(c(first.date, last.date), c(0, 1), type = "n",
    ...
    if(cycle == 21) legend(legend = c("North", "South", "Both"), 
                           bg = "white", 
                           pch = c(19, 17, 4), 
                           lty = c(1, 1, 2), 
    ...
    for (hemisphere in c("N", "S", "all")){
      ...
      c_alpha <- get_c_alpha(data_cir = data_cir, data_lin = data_lin, 
                             h = h, g = g,
                             alpha = alpha)
      ...
      if (hemisphere != "all"){
        ...
        kde_vec <- kde_cir_lin(th = thz[, 1], z = thz[, 2],
        ...
        if(cycle == 21) axis(2L, at = c(-pi,-pi/2,0, pi/2,pi),
                             c("-180","-90", "0", "90", "180"))
        
        col <- c(sequential_hcl(length(c_alpha)-1, 
                                if (hemisphere == "N") "Blues 3" else "Reds 3"), 
                 "#FFFFFF")
        .filled.contour(z, th, t(kde_mat), levels = rev(c_alpha),
                        col = rev(col))
        contour(x = z, y = th, z = t(kde_mat), 
    ...
    par(mar = c(1, if(cycle == 21) 4 else -0.1, -0.1, 
                if(cycle == 23) 2 else -0.1) + 0.1)
    
    plot(c(first.date, last.date), c(-pi, pi), type = "n",
         ylab = "Longitudes (º)",
         xlab = "Date",
         yaxt = "n"
         )
